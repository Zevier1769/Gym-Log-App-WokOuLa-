...
public class SQLiteManager extends SQLiteOpenHelper
{
    ...
    private static final String TARIK_FIELD = "tarik";
...
//        }
    ...
    public void addNoteToDatabase(Note note)
    {
        ...
        contentValues.put(TARIK_FIELD, note.getTarik());
        contentValues.put(TITLE_FIELD, note.getTitle());
        contentValues.put(DESC_FIELD, note.getDesc());
        contentValues.put(LOG_FIELD, note.getLog());
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if(result.getCount() != 0)
            {
                while (result.moveToNext())
                {
                    ...
                    String tarik = result.getString(2);
                    String title = result.getString(3);
                    String desc = result.getString(4);
                    String log = result.getString(5);
                    String stringDeleted = result.getString(6);
                    Date deleted = getDateFromString(stringDeleted);
                    Note note = new Note(id,tarik,title,desc,log,deleted);
